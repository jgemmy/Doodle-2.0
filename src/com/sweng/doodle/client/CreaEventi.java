...
public class CreaEventi {
	...
	public CreaEventi(final TabPanel pannello){
	...
	public void inEvento(){
		Evento event = new Evento( "" ,snome, sluogo, sdescs, ifrom, ito,Doodle_Main.idKey,1,"");
//		System.out.println(event.getIdKey());
		greetingService.caricaEvento(event , new AsyncCallback<String>() {
	...
	public void inInsertJoin(){
				greetingService.insertJoin(new Partecipante( idevento,"", Doodle_Main.idKey, "Owner", ""+1), new AsyncCallback<String>() {
		
					@Override
					public void onSuccess(String result) {
						Window.alert("Evento result insertJOin = "+result);
